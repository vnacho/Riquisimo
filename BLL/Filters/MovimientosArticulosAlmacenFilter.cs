...
{
    public class MovimientosArticulosAlmacenFilter
    {
        //[Display(Name = "Fecha")]
        ...
        public Expression<Func<MovimientosArticulosAlmacen, bool>> ExpressionFilter()
        {
            return f =>
                (ArticulosAlmacenId.HasValue ? f.ArticulosAlmacenId == ArticulosAlmacenId : true) &&
                (FechaDesde.HasValue ? f.FechaMovimiento >= FechaDesde : true) &&
                (FechaHasta.HasValue ? f.FechaMovimiento <= FechaHasta : true) &&
                (string.IsNullOrWhiteSpace(RateFiltro) ? true : f.ArticulosAlmacen.Rate.Contains(RateFiltro)) &&
                (CentroId.HasValue ? f.CentroCosteId == CentroId : true) &&
                (string.IsNullOrWhiteSpace(Tipo) ? true : f.movimiento.Contains(Tipo));
        }
    }
}
