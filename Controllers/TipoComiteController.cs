...
namespace Ferpuser.Controllers
{
    ...
    public class TipoComiteController : Controller
    {
        private readonly SageContext _sageContext;
        private readonly SageComuContext _sageComuContext;
        private readonly ApplicationDbContext _dbContext;

        public TipoComiteController(SageContext sageContext, SageComuContext sageComuContext, ApplicationDbContext dbContext)
        {
        ...
        public async Task<IActionResult> Index(int? pag, string sort = "", string currentsort = "", bool reset = false, bool export = false)
        {
            ...
            TipoComiteFilter filter = new TipoComiteFilter();
            await TryUpdateModelAsync<TipoComiteFilter>(filter, "filter",
                f => f.Nombre);

            Pager pager = new Pager(await _dbContext.TiposComite.CountAsync(filter.ExpressionFilter()), pag, 10, 5);
            ViewData["Pager"] = pager;
            ViewData["Sort"] = sort;

            IEnumerable<TipoComite> list = await _dbContext.TiposComite.Where(filter.ExpressionFilter())
                .OrderBy(sort)
        ...
        {
            TipoComite model = new TipoComite();
            await TryUpdateModelAsync<TipoComite>(model, "",
                f => f.Id,
                f => f.Nombre);

            //Validación de negocio            
            TipoComiteValidator validator = new TipoComiteValidator(_dbContext);
            IEnumerable<ValidationResult> businessErrors = validator.Create(model);
            this.AddValidationErrors(businessErrors);

            if (ModelState.IsValid)
            {
                TipoComiteManager manager = new TipoComiteManager(_dbContext);
        ...
        {
            var model = _dbContext.TiposComite.Single(f => f.Id == id);
            return View(model);
        }
        ...
        {
            TipoComite model = _dbContext.TiposComite.Find(id);
